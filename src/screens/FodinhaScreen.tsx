    ...
    <View style={{ flex: 1 }} onLayout={handleLayout}>
            ...
            {(() => {
                ...
                return (
                    ...
                    >
                        ...
                        >
                            <Ionicons name="remove" size={ms(14)} color={isCardsLocked ? theme.colors.text.secondary : theme.colors.text.white} />
                        </TouchableOpacity>

                        <View style={styles.cardsCenter}>
                            <Ionicons name="documents-outline" size={ms(12)} color={theme.colors.text.white} style={{ marginRight: ms(4) }} />
                        ...
                        <TouchableOpacity
                            ...
                            disabled={isCardsLocked}
                        >
                            <Ionicons name="add" size={ms(14)} color={isCardsLocked ? theme.colors.text.secondary : theme.colors.text.white} />
                        </TouchableOpacity>
                    </View>
                );
            })()}
                    ...
                    >
                        ...
                        {game.roundPhase === 'results' && (
                            <View style={styles.feedbackContainer}>
                                <Text style={{ fontSize: ms(8), color: theme.colors.text.secondary, fontFamily: 'Minecraft' }}>{translate('fodinha.bet')}: {p.currentBid}</Text>
                                {projectedDamage > 0 ? (
                                    <Text style={{ fontSize: ms(10), color: theme.colors.fodinha.damageText, fontFamily: 'Minecraft' }}>-{projectedDamage}</Text>
                                ) : (
                                    <Ionicons name="checkmark-circle" size={ms(12)} color={theme.colors.status.success} />
                                )}
                            </View>
                        )}
                    </View>
      ...
      {showEditHistory && editingRoundIdx !== null && (
        <View style={[styles.absoluteOverlay, { backgroundColor: theme.colors.background.overlay }]}>
            ...
            <View style={[styles.overlayContent, { width: wp('50%'), maxHeight: '85%', backgroundColor: theme.colors.background.secondary, borderColor: theme.colors.text.secondary }]}>
                ...
                <View style={styles.modalHeader}>
                    ...
                    <TouchableOpacity onPress={() => setShowEditHistory(false)}><Ionicons name="close" size={ms(24)} color={theme.colors.text.primary} /></TouchableOpacity>
                </View>

                <View style={{ flexShrink: 1 }}>
                    <ScrollView showsVerticalScrollIndicator={true} contentContainerStyle={{ paddingBottom: ms(20) }}>
                        {game.players.map(p => {
                            ...
                            return (
                                <View key={p.id} style={[styles.editHistoryRow, { borderBottomColor: theme.colors.fodinha.divider }]}>
                                    <Text style={{ color: theme.colors.text.primary, fontFamily: 'Minecraft', fontSize: ms(12), flex: 1 }}>{p.name}</Text>
                                    <View style={[styles.stepperContainer, { backgroundColor: theme.colors.background.overlay }]}>
                ...
                <View style={[styles.modalFooterRow, { borderTopColor: theme.colors.fodinha.divider }]}>
                        <TouchableOpacity onPress={() => game.deleteRound(editingRoundIdx, () => setShowEditHistory(false))} style={{ padding: ms(10) }}>
                           <Text style={{ color: theme.colors.status.error, fontFamily: 'Minecraft', fontSize: ms(12), textDecorationLine: 'underline' }}>{translate('cacheta.delete_round')}</Text>
                        </TouchableOpacity>
...
const styles = StyleSheet.create({
  safeArea: { flex: 1 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: ms(20), height: ms(50) },
  headerSide: { flex: 1, flexDirection: 'row' },
  headerTitle: { fontFamily: 'Minecraft', color: '#FFF', fontSize: ms(16), textAlign: 'center', flex: 2 },
  iconBtn: { padding: ms(5) },
  cardsControlContainer: { flexDirection: 'row', alignItems: 'center', borderRadius: ms(10), borderWidth: 1, height: ms(30) },
  cardsStepBtn: { height: '100%', paddingHorizontal: ms(8), justifyContent: 'center', alignItems: 'center' },
  ...
  playerCell: { height: ms(60), paddingHorizontal: ms(10), justifyContent: 'center', marginBottom: ms(4), borderRadius: ms(10) },
  ...
  historyCell: { width: ms(34), height: ms(60), justifyContent: 'center', alignItems: 'center', marginBottom: ms(4), borderRadius: ms(6) },
  safeDot: { width: ms(8), height: ms(8), borderRadius: ms(4) },
  damageText: { fontFamily: 'Minecraft', fontSize: ms(12) },
  activeColumn: { marginLeft: ms(15), borderRadius: ms(12), overflow: 'hidden', borderWidth: 1, alignSelf: 'flex-start', minWidth: ms(160) },
  cellHeaderActive: { height: ms(30), justifyContent: 'center', alignItems: 'center' },
  activeCell: { height: ms(60), flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: ms(10), marginBottom: ms(4), borderBottomWidth: 1 },
  stepperContainer: { flexDirection: 'row', alignItems: 'center', gap: ms(10), borderRadius: ms(8), padding: ms(4), minWidth: ms(120), justifyContent: 'center' },
  stepBtn: { width: ms(24), height: ms(24), justifyContent: 'center', alignItems: 'center', borderRadius: ms(4) },
  ...
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: ms(10) },
  editHistoryRow: { flexDirection: 'row', alignItems: 'center', marginBottom: ms(8), paddingBottom: ms(8), borderBottomWidth: 1 },
